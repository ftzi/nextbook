...
Make nextbook the first component documentation tool with built-in AI assistant support. The CLI generates `CLAUDE.md` and `AGENTS.md` files that teach AI assistants how to write stories, enabling instant story generation with best practices.
...
By generating AI instruction files, nextbook becomes:
...
The marker pattern (`<!-- NEXTBOOK:START -->...<!-- NEXTBOOK:END -->`) allows:
- Fresh installs get complete instructions
- Re-running `npx nextbook` upgrades the nextbook section
- Users can add custom instructions outside markers (project-specific patterns)

## What Changes

### packages/nextbook (CLI)
...
- `.nextbook/` hidden directory: Less discoverable
...
**Choice:** Use HTML comments `<!-- NEXTBOOK:START -->` and `<!-- NEXTBOOK:END -->`
...
- Re-running CLI updates only the nextbook section
...
- [x] Create marker-aware file writer utility
  ...
  - Find `<!-- NEXTBOOK:START -->` and `<!-- NEXTBOOK:END -->` markers
...
- [x] Update comparison table
  - Add row: "AI Assistant Support" — Storybook: No, Nextbook: Yes (with checkmark)
...
- `[NEX.cli.init]` — MODIFIED (generates AI instruction files)
- `[NEX.cli.ai-instructions]` — ADDED (new feature)
