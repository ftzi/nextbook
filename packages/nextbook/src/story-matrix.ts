 ...
 * @returns A matrix story object that Nextbook renders as a grid of all combinations
 */
...
/**
 * Type guard to check if a value is a Nextbook matrix story.
 */
export function isMatrixStory(value: unknown): value is MatrixStory<z.ZodObject<z.ZodRawShape>> {
	return (
		...
		value !== null &&
		"__nextbook" in value &&
		value.__nextbook === true &&
		"__nextbook_matrix" in value &&
		value.__nextbook_matrix === true
	)
}
