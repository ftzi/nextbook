 ...
 * export { generateStorifyParams as generateStaticParams } from '@workspace/storify'
 *
 ...
 */
export async function generateStorifyParams(storiesDir?: string): Promise<StorifyParams[]> {
	...
	const modulePromises = files.map(async (file) => {
		try {
			...
			console.warn(`[storify] Could not load story: ${file}`, error)
...
export function createGenerateParams(options?: { storiesDir?: string }) {
	return () => generateStorifyParams(options?.storiesDir)
}
