...
describe("storify init", () => {
	...
	test("uses npm imports by default", () => {
		...
		expect(layout).toContain('from "@ftzi/storify"')
		expect(layout).not.toContain("@workspace/storify")
	})
	...
	test("uses workspace imports when --workspace flag is set", () => {
		...
		const layout = readFileSync(join(TEST_DIR, "app/ui/layout.tsx"), "utf-8")
		expect(layout).toContain('from "@workspace/storify"')
	})
	...
	test("overwrites existing files when skipExisting is false", () => {
		...
		const content = readFileSync(join(TEST_DIR, "app/ui/layout.tsx"), "utf-8")
		expect(content).toContain("StorifyShell")
	})
	...
	test("layout contains StorifyShell", () => {
		mkdirSync(join(TEST_DIR, "app"), { recursive: true })

		init({ cwd: TEST_DIR })

		const layout = readFileSync(join(TEST_DIR, "app/ui/layout.tsx"), "utf-8")
		expect(layout).toContain("StorifyShell")
	...
	test("CLAUDE.md and AGENTS.md contain AI assistant instructions with markers", () => {
		...
		for (const content of [claude, agents]) {
			expect(content).toContain("<!-- STORIFY:START -->")
			expect(content).toContain("<!-- STORIFY:END -->")
			expect(content).toContain("Storify")
...
`
		...
		// Old storify content should be replaced
		expect(claude).not.toContain("Old Storify Content")
		expect(claude).not.toContain("This will be replaced.")

		// New storify content should be present
	...
	test("full installation flow: fresh install, upgrade, and customization preserved", () => {
		...
		expect(initialClaude).toContain("<!-- STORIFY:START -->")
		expect(initialClaude).toContain("<!-- STORIFY:END -->")
...
`
		...
		// Storify content still present
		expect(upgradedClaude).toContain("<!-- STORIFY:START -->")
		expect(upgradedClaude).toContain("<!-- STORIFY:END -->")
		expect(upgradedClaude).toContain("storyMatrix")

		// Step 5: Verify other files weren't overwritten
		const layout = readFileSync(join(TEST_DIR, "app/ui/layout.tsx"), "utf-8")
		expect(layout).toContain("StorifyShell") // Original content intact
	})
})
